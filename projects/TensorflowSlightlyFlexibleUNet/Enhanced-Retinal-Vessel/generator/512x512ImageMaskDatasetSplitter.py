...
#
# ImageMaskDatsetSplitter.py

# 2023/08/20 Toshiyuki Arai @antillia.com
...
class ImageMaskDatasetGenerator:
  ...
  def create_dataset(self, image_files, output_dir, dataset, debug=False):
    ...
    for image_file in image_files:
      ...
      output_filepath = os.path.join(output_images_dir, basename)
      resized = self.resize(image_file, mask=False)
      ...
      resized_mask  = self.resize(mask_filepath, mask=True)
      output_mask_filepath  = os.path.join(output_masks_dir, basename)
      resized_mask.save(output_mask_filepath)
      print("Saved {}".format(output_mask_filepath))

  def resize(self, image_file, mask=False):
